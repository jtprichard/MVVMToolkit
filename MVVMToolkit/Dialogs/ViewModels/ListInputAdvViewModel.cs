...
{
    public class ListInputAdvViewModel : BaseDialogViewModel
    {
        #region Private Fields

        private const int DefaultListId = 1000;

        private readonly ObservableCollection<IListItemAdv> _originalItemList = null;
        ...
        /// </summary>
        public ObservableCollection<IListItemAdv> ListItems { get; private set; }

        /// <summary>
        /// Instance of viewmodel for databinding
        /// </summary>
        internal static ListInputAdvViewModel Instance { get; set; }

        protected ObservableCollection<IListItemAdv> _visibleListItems;
        /// <summary>
        ...
        /// </summary>
        public virtual ObservableCollection<IListItemAdv> VisibleListItems
        {
            get { return _visibleListItems; }
            set { _visibleListItems = value; OnPropertyChanged(nameof(VisibleListItems)); }
        }

        protected IListItemAdv _selectedListItem;
        /// <summary>
        /// The selected list item
        /// </summary>
        public IListItemAdv SelectedListItem
        {
        ...
        private IEnumerable<IListItemAdv> _comboboxItems;
        /// <summary>
        ...
        /// </summary>
        public IEnumerable<IListItemAdv> ComboboxItems
        {
            get { return _comboboxItems; }
            set { _comboboxItems = value; OnPropertyChanged(nameof(ComboboxItems)); }
        }

        private IListItemAdv _selectedComboboxItem;
        /// <summary>
        /// The selected combobox item.
        /// </summary>
        public IListItemAdv SelectedComboboxItem
        {
        ...
        public ListInputAdvViewModel(IEnumerable<IListItemAdv> listItems, string message, string itemType, int defaultId = 0)
        {
            ...
            var tempItems = ListItemAdv.Clone(listItems);
            if (tempItems.Count == 0 || (tempItems.Count == 1 && tempItems.FirstOrDefault().Id == -1))
            {
                _dummyListItem = (ListItemAdv)tempItems.FirstOrDefault();
                ListItems = new ObservableCollection<IListItemAdv>();
                _originalItemList = new ObservableCollection<IListItemAdv>();
                _listId = DefaultListId;
            }
            else
            {
                ListItems = ListItemAdv.Clone(listItems);
                _dummyListItem = (ListItemAdv)ListItems.FirstOrDefault();
        ...
        protected ObservableCollection<DialogInput> CreateDialogInputs(IListItemAdv item = null)
        {
            //Create Inputs
        ...
        protected void AddItemToList(ObservableCollection<DialogInput> inputs, int id)
        {
            ...
            var answer = inputs.FirstOrDefault(x => x.Description == nameof(ListItemAdv.Description))?.Answer;
            var customPropertyInputs = inputs.Where(x => x.Description != nameof(ListItemAdv.Description));
        ...
        protected void EditItemOnList(IListItemAdv item, ObservableCollection<DialogInput> inputs)
        {
            ObservableCollection<ListItemProperty> properties = new ObservableCollection<ListItemProperty>();
        ...
        /// <returns></returns>
        private ListItemAdv GetSelectedListItem(IListItemAdv item)
        {
            var returnedItem = ListItems.FirstOrDefault(x => x.Id == item.Id);

            return (ListItemAdv)ListItems.FirstOrDefault(x => x.Id == item.Id);
        ...
        protected virtual IListItemAdv CreateListItem(IListItemAdv item, IListItemAdv parent = null)
        {
            var listItem = new ListItemAdv(item.Description, item.Id, item.IsLocked);
            listItem.Parent = parent;
            listItem.AddCustomProperties(item.CustomProperties);
            return listItem;
        }
